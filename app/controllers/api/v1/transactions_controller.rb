  ...
  module V1
      ...
      def update
        if @transaction.update(transaction_params)
          transaction = Transaction.update_trans_status(@transaction.status_trigger,@transaction.id)
          render json: transaction, status: :created
        else
          render json: @transaction.errors, status: :unprocessable_entity
        end
      ...
      private
        ...
        def transaction_params
          params.require(:transaction).permit(:transaction_id, :customer_id, :input_currency, :input_amount, :output_currency, :output_amount, :trans_status, :active_status, :created_at, :status_trigger)
        end
    end
  end
end
